...
export default class USB extends EventEmitter {
          ...
          if ('win32' !== os.platform()) {
            if (iface.isKernelDriverActive()) {
              try {
                ...
                console.error('[FAIL] Could not detatch kernel driver: %s', e)
              }
            }
          }
  ...
  close(callback) {
    if (!this.device) {
      log(`escpos-usb: No device`, { prefix: '[FAIL]' })
      ...
      try {
        ...
        log(`escpos-usb: ${err ? err.message || err.name : 'error catched.'}`, { prefix: '[FAIL]', skip: true })
      }
    }
    return this
  }
   ...
   * @returns {USB}
   */
  removeDetachListeners(callback) {
    if (!this.device) {
      log(`escpos-usb: No device`, { prefix: '[FAIL]' })
      if (typeof callback === 'function') callback()
    } else {
    ...
    return new Promise((resolve, reject) => {
      if (!this.device) {
        ...
        try {
          ...
          log(reason, { prefix: '[FAIL]', skip: true })
          reject(reason)
        }
      }
    })
