...
;(function () {
  ...
  if (typeof require !== 'undefined') {
    ...
    decoder = require('node:string_decoder')
    ...
    const transform = new stream.Transform({
      ...
      flush(callback) {
        ...
        switch (state.line) {
          ...
          case 'horizontal':
            // append commands to stop rules
            const s = ptr.command.normal() + ptr.command.area(state.rules.left, state.rules.width, state.rules.right) + ptr.command.align(0) + ptr.command.vrstop(state.rules.widths) + ptr.command.vrlf(false)
            ptr.upsideDown ? this.buffer.push(s) : this._push(s)
            state.line = 'waiting'
            break
          default:
  ...
  function createLine(line, printer, state) {
    ...
    if ('image' in column) {
      ...
      result.push(printer.command.normal() + printer.command.area(left, width, right) + printer.command.align(column.align) + printer.command.image(column.image))
    }
      ...
      for (let z = 0; z < img.height; z += this.split) {
        ...
        for (let y = 0; y < h; y++) {
          ...
          for (let x = 0; x < w; x += 8) {
            ...
            for (let p = 0; p < q; p++) {
              const f = Math.floor((d[i] + e * 5) / 16 + Math.pow(((img.data[j] * 0.299 + img.data[j + 1] * 0.587 + img.data[j + 2] * 0.114 - 255) * img.data[j + 3] + 65525) / 65525, 1 / this.gamma) * 255)
