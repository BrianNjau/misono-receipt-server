...
const receiptline = require('./receiptline.js')
const iconv = require('iconv-lite')
const PNG = undefined // require('pngjs').PNG
                  ...
                  if ((buf[0] & 0xf1) === 0x21) {
                    ...
                    if (l <= buf.length) {
                      ...
                      if ((buf[2] & 0x20) === 0x20) {
                        ...
                        // write command
                        drain = conn.write(
                          (await transform(receiptmd, printer))
                            .replace(/^(\x1b@)?\x1b\x1ea\x00/, '$1\x1b\x1ea\x01\x17') // (ESC @) ESC RS a n ETB
                            .replace(/(\x1b\x1d\x03\x01\x00\x00\x04?|\x1b\x06\x01)$/, '\x17'),
                          'binary',
                        ) // ETB
                        // set timer
                        tid = setTimeout(() => close('timeout'), timeout * 1000)
      ...
      for (let z = 0; z < 24; z++) {
        if (y + z < h) {
          ...
          for (let x = 0; x < w; x += 8) {
            ...
            const q = Math.min(w - x, 8)
            for (let p = 0; p < q; p++) {
              const f = Math.floor((d[i] + e * 7) / 16 + Math.pow(((img.data[j] * 0.299 + img.data[j + 1] * 0.587 + img.data[j + 2] * 0.114 - 255) * img.data[j + 3] + 65525) / 65525, 1 / this.gamma) * 255)
              j += 4
              if (this.gradient) {
                d[i] = e * 3
                e = f < this.threshold ? ((b |= 128 >> p), f) : f - 255
